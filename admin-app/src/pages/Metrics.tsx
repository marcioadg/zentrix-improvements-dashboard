...
export const Metrics = () => {
  ...
  } = useUserSettings(selectedTeamId);
  ...
  const handleCreateIssueFromMetric = useCallback(async (title: string, description: string, ownerId?: string): Promise<void> => {
    logger.log('📝 Metrics Page - Creating issue from metric:', { title, description, ownerId });
    try {
      const success = await addIssue(title, description, 'short_term', ownerId);
      if (success) {
        logger.log('✅ Metrics Page - Issue created successfully from metric');
      }
    } catch (error) {
      logger.error('❌ Metrics Page - Failed to create issue from metric:', error);
    }
  }, [addIssue]);
  ...
  return (
    <>
      ...
      {user && !loading && teams.length === 0 && (
        <div className="space-y-4 md:space-y-6 px-6 py-6">
          <div className="flex flex-col gap-2 md:gap-1">
            <div>
              <h1 className="text-[20px] font-semibold text-foreground tracking-tight">Metrics</h1>
      ...
      {user && !loading && teams.length > 0 && (
        <div className="flex flex-col h-[calc(100dvh-3rem)] overflow-hidden space-y-4 md:space-y-6 px-6 py-6">
          <div className="flex flex-col gap-2 md:gap-1">
            <div>
              <h1 className="text-[20px] font-semibold text-foreground tracking-tight">Metrics</h1>
              <p className="text-[13px] text-muted-foreground mt-0.5">
          ...
          {selectedTeamId && selectedTeamId.trim() !== '' && (
            <div data-tour="metrics-content" className="flex-1 min-h-0 flex flex-col">
