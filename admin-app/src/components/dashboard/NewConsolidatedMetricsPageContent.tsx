...
import { NewWeeklyMetricsTable } from '@/components/dashboard/table/NewWeeklyMetricsTable';
...
interface NewConsolidatedMetricsPageContentProps {
  teams: any[];
  selectedTeam: string;
...
export const NewConsolidatedMetricsPageContent: React.FC<NewConsolidatedMetricsPageContentProps> = React.memo(({
  teams,
  selectedTeam,
  setSelectedTeam,
  userSettings,
  ...
  const handleCreateIssue = useCallback(async (title: string, description: string, ownerId?: string) => {
    ...
    if (onCreateIssue) {
      logger.debug('NewConsolidatedMetricsPageContent: Using custom issue creation handler (meeting context)');
    ...
    try {
      ...
      if (userError || !user) {
        logger.error('❌ NewConsolidatedMetricsPageContent: Authentication error:', userError);
        toast({
      ...
      logger.debug('NewConsolidatedMetricsPageContent: Issue created successfully');
      
    } catch (error) {
      logger.error('❌ NewConsolidatedMetricsPageContent: Failed to create issue:', error);
      toastRef.current({
        title: "Error",
  ...
  const handleAddMetric = useCallback(async (metricData: {
    ...
    logger.debug('NewConsolidatedMetricsPageContent: Adding single metric via AddMetricModal', metricData);
    ...
    try {
      ...
      logger.debug('NewConsolidatedMetricsPageContent: Metric creation completed');
      ...
      setShowAddMetricModal(false);
      logger.debug('NewConsolidatedMetricsPageContent: Single metric added successfully');
    } catch (error) {
      logger.error('❌ NewConsolidatedMetricsPageContent: Failed to add single metric:', error);
      toastRef.current({
        title: "Error",
