...
import { Settings, Archive, Calendar, Clock, User } from 'lucide-react';
...
export const KanbanSortControls: React.FC<KanbanSortControlsProps> = ({
  preferences,
  onUpdatePreferences,
}) => {
  ...
  const handleShowArchivedChange = (checked: boolean) => {
    logger.log('KanbanSortControls: Show archived changed to:', checked);
    onUpdatePreferences({ showArchived: checked });
  };

  const handleMyTasksOnlyChange = (checked: boolean) => {
    logger.log('KanbanSortControls: My tasks only changed to:', checked);
