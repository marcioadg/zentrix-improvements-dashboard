...
interface EditFastTaskModalProps {
  open: boolean;
...
export const EditFastTaskModal: React.FC<EditFastTaskModalProps> = ({
  open,
  onOpenChange,
  task,
  onUpdate,
  ...
  useEffect(() => {
    logger.log('🔍 EditFastTaskModal - Task received:', {
      taskId: task.id,
      title: task.title,
      ...
      assignedToAvatarUrl: task.assignedToAvatarUrl,
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      logger.log('🔧 EditFastTaskModal: Updating task', {
        taskId: task.id,
        taskType,
        selectedTeamId,
        assignedTo
      ...
      logger.log('🔧 EditFastTaskModal: Team resolution:', {
        selectedTeamId,
        resolvedTeamName,
        taskType,
      ...
      logger.log('🔧 EditFastTaskModal: Final updates:', updates);
      
      await onUpdate(task.id, updates);
      onOpenChange(false);
    } catch (error) {
      logger.error('❌ EditFastTaskModal: Failed to update task:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent 
        className={cn(
          "flex flex-col sm:max-w-lg max-h-[90vh]",
          dialogClassName
        )}
      >
        {/* Fixed Header */}
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-4 space-y-4 min-h-0 w-full max-w-full">
